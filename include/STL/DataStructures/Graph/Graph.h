...
class MinInfo
{
    public:
        ...
        friend bool operator< (const MinInfo& lhs, const MinInfo& rhs)
...
class Graph
{
    public:
        ...
        template <typename U>
        friend int ShortestPath(Graph<U>& g, const U& sVertex,
        ...
        template <typename U>
        friend int MinimumPath(Graph<U>& g, const U& sVertex, const U& eVertex,
                               std::list<U>& path);

        // find the minimum spanning tree for the strongly connected digraph g
        template <typename U>
        friend int MinSpanTree(Graph<U>& g, Graph<U>& MST);
        ...
        template <typename U>
        friend void TopologicalSort(Graph<U>& g, std::list<U>& tlist);
        ...
        template <typename U>
        friend void StrongComponents(Graph<U>& g, std::vector<std::set<U> >& component);
    ...
    private:
        ...
        // it detects a cycle
        template <typename U>
        friend void DfsVisit(Graph<U>& g, const U& sVertex, std::list<U>& dfsList,
                                    bool checkForCycle);
};
...
void Graph<T>::eraseVertex(const T& v)
{
    ...
    typename vertexMap::iterator mIter = vtxMap.find(v);
